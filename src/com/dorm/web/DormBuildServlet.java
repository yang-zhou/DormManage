...
public class DormBuildServlet extends HttpServlet{
	...
	private void dormBuildDelete(HttpServletRequest request,
		...
		try {
			...
			if(dormBuildDao.existManOrDormWithId(con, dormBuildId)) {
				request.setAttribute("error", "管理员未移除或宿舍楼不存在！");
			} else {
	...
	private String genPagation(int totalNum, int currentPage, int pageSize){
		...
		if(currentPage==1) {
			...
			pageCode.append("<li><a href='dormBuild?page="+(currentPage-1)+"'><</a></li>");
		}
		...
		pageCode.append("<li><a href='dormBuild?page="+totalPage+"'>末页</a></li>");
